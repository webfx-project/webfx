 ...
 * @author Bruno Salmon
 */
public class SwingDataGridPeer
        <N extends DataGrid, NB extends DataGridPeerBase<Object, N, NB, NM>, NM extends DataGridPeerMixin<Object, N, NB, NM>>

        extends SwingRegionPeer<N, NB, NM>
        implements DataGridPeerMixin<Object, N, NB, NM>, SwingLayoutMeasurable<N> {
    ...
    public SwingDataGridPeer() {
        this((NB) new DataGridPeerBase());
    }

    public SwingDataGridPeer(NB base) {
        super(base);
    ...
    private TableCellRenderer createTableCellRenderer(ValueRenderer valueRenderer, DisplayColumn displayColumn, boolean header) {
        return valueRenderer == null ? null : (jTable, value, isSelected, hasFocus, row, column) -> {
            ...
            String rowStyle = getNodePeerBase().getRowStyle(row);
            StyleUtil.styleCellComponent(cellComponent, rowStyle, SwingPaints.toSwingPaint(getNodePeerBase().getRowBackground(row)), header, textAlign, isSelected);
    ...
    private class DisplayTableModel extends AbstractTableModel {
        ...
        public void setDisplayResultSet(DisplayResultSet rs) {
            ...
            this.rs = rs;
            columnCount = getNodePeerBase().getGridColumnCount();
            rowStyleColumnIndex = getNodePeerBase().getRowStyleColumnIndex();
        }
        ...
        public Object getValueAt(int rowIndex, int columnIndex) {
            return rs == null ? null : rs.getValue(rowIndex, getNodePeerBase().gridColumnIndexToResultSetColumnIndex(columnIndex, rowStyleColumnIndex));
        }
    }
}
