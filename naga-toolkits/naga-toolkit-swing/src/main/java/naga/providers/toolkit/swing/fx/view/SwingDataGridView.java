 ...
 * @author Bruno Salmon
 */
public class SwingDataGridView
        extends SwingRegionView<DataGrid, DataGridViewBase<Object>, DataGridViewMixin<Object>>
        implements DataGridViewMixin<Object>, SwingLayoutMeasurable<DataGrid> {
    ...
    public void updateResultSet(DisplayResultSet rs) {
        DataGridViewBase<Object> base = getNodeViewBase();
        base.initGrid(rs);
    ...
    private TableCellRenderer createTableCellRenderer(ValueRenderer valueRenderer, DisplayColumn displayColumn, boolean header) {
        return valueRenderer == null ? null : (jTable, value, isSelected, hasFocus, row, column) -> {
            ...
            String rowStyle = getNodeViewBase().getRowStyle(row);
    ...
    private class DisplayTableModel extends AbstractTableModel {
        ...
        public int getColumnCount() {
            return getNodeViewBase().getGridColumnCount();
        }

        @Override
        public Object getValueAt(int rowIndex, int columnIndex) {
            DisplayResultSet rs = getNode().getDisplayResultSet();
            return rs == null ? null : rs.getValue(rowIndex, getNodeViewBase().gridColumnIndexToResultSetColumnIndex(columnIndex));
        }
    }
}
