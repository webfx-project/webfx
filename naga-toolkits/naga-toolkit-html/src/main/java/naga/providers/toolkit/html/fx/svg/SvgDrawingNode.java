...
public class SvgDrawingNode extends HtmlParent</*SVGElement*/ Element> implements DrawingNode, DrawingMixin {
    ...
    public SvgDrawingNode(Element container) {
        super(container);
        if (!container.hasAttribute("width"))
            container.setAttribute("width", "100%");
        if (!container.hasAttribute("height"))
            container.setAttribute("height", "600px");
        heightProperty().addListener((observable, oldValue, newHeight) -> container.setAttribute("height", "" + newHeight + "px"));
