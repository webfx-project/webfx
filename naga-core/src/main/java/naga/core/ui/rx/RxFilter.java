...
public class RxFilter {
    ...
    public RxFilter addRowStylesExpressionColumn() {
        DomainClass domainClass = dataSourceModel.getDomainModel().getClass(domainClassId);
        ExpressionArray rowStylesExpressionArray = domainClass.getRowStylesExpressionArray();
        if (rowStylesExpressionArray != null) {
            ExpressionColumn[] expressionColumns2 = new ExpressionColumn[expressionColumns.length + 1];
            expressionColumns2[0] = ExpressionColumn.create(rowStylesExpressionArray, new DisplayColumn(null, "css", PrimType.STRING, "css"));
