...
public class ReliableSubscribeBus extends BusProxy {
    ...
    public ReliableSubscribeBus(Bus delegate, JsonObject options) {
        super(delegate);
        sequenceNumberKey = options == null || !options.has(SEQUENCE_NUMBER) ? "v" : options.getString(SEQUENCE_NUMBER);
        publishChannel = options == null || !options.has(PUBLISH_CHANNEL) ? "realtime/store" : options.getString(PUBLISH_CHANNEL);
        acknowledgeDelayMillis = options == null || !options.has(ACKNOWLEDGE_DELAY_MILLIS) ? 3 * 1000 : (int) options.getNumber(ACKNOWLEDGE_DELAY_MILLIS);
    ...
    public void synchronizeSequenceNumber(String topic, double initialSequenceNumber) {
        assert !currentSequences.has(topic) && !knownHeadSequences.has(topic) && !pendings.has(topic);
    ...
    protected boolean onReceiveMessage(Message<?> message) {
        ...
        if (sequence > currentSequence + 1) {
            ...
            log.log(Level.CONFIG, "Missed message, current sequence=" + currentSequence + " incoming sequence=" + sequence);
    ...
    private void scheduleAcknowledgment(final String topic) {
        if (!acknowledgeScheduled.has(topic)) {
            ...
            Platform.scheduleDelay(acknowledgeDelayMillis, event -> {
                if (acknowledgeScheduled.has(topic)) {
                    acknowledgeScheduled.remove(topic);
                    // Check we're still out of date, and not already catching up.
                    double knownHeadSequence = knownHeadSequences.getNumber(topic);
                    double currentSequence = currentSequences.getNumber(topic);
                    if (knownHeadSequence > currentSequence && (!acknowledgeNumbers.has(topic) || knownHeadSequence > acknowledgeNumbers.getNumber(topic))) {
    ...
    private void scheduleMessages(final JsonArray messages) {
        Platform.scheduleDeferred(event -> messages.forEach(new JsonArray.ListIterator<Message<?>>() {
            @Override
