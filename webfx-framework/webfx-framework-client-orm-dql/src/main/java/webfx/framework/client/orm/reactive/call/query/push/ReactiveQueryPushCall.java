...
public final class ReactiveQueryPushCall extends ReactiveQueryCall {
    ...
    private final List<ReactiveQueryPushCall> children = new ArrayList<>();
    ...
    private final ObjectProperty<ReactiveQueryPushCall> parentProperty = new SimpleObjectProperty<ReactiveQueryPushCall/*GWT*/>() {
        private ReactiveQueryPushCall previousParent;
        @Override
        protected void invalidated() {
            if (previousParent != null)
                previousParent.children.remove(ReactiveQueryPushCall.this);
            ReactiveQueryPushCall newParent = get();
            if (newParent != null)
                newParent.children.add(ReactiveQueryPushCall.this);
            previousParent = newParent;
            scheduleFireCallNowIfRequired();
        }
    };

    public ReactiveQueryPushCall getParent() {
        return parentProperty.get();
    }

    public ReactiveQueryPushCall setParent(ReactiveQueryPushCall parent) {
    ...
    protected boolean isFireCallRequiredNow() {
        ...
        ReactiveQueryPushCall parent = getParent();
    ...
    protected void callAsyncFunction() {
        ...
        QueryPushService.executeQueryPush(new QueryPushArgument(
                ...
                resend,
                null,
                queryPushResult -> { // This consumer will be called each time the server will notify a change on the result
                    QueryResult queryResult = computeQueryResult(queryPushResult);
                    onCallResult(queryResult, null);
            ...
            if (refreshChildren)
                children.forEach(ReactiveQueryPushCall::fireCallWhenReady);
        });
