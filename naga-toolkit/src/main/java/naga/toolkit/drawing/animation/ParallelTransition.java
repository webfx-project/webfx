...
import static naga.toolkit.drawing.animation.TickCalculation.add;
import static naga.toolkit.drawing.animation.TickCalculation.fromDuration;
import static naga.toolkit.drawing.animation.TickCalculation.sub;
...
public final class ParallelTransition extends Transition {
    ...
    private Duration computeCycleDuration() {
        ...
        for (Animation animation : getChildren()) {
            ...
            Duration totalDuration = (absRate < EPSILON) ?
                    animation.getTotalDuration() : Duration.ofMillis((long) (animation.getTotalDuration().toMillis() / absRate));
             Duration childDuration = totalDuration.plus(animation.getDelay());
            if (childDuration == INDEFINITE_DURATION) {
                return INDEFINITE_DURATION;
            } else {
                if (childDuration.compareTo(maxTime) > 0) {
    ...
    void impl_start(boolean forceSync) {
        ...
        final long currentTicks = fromDuration(getCurrentTime());
        if (curRate < 0) {
            jumpToEnd();
        ...
        for (final Animation animation : cachedChildren) {
            ...
            if (newTicks <= delays[i]) {
                ...
                if (status != Status.STOPPED) {
                    animation.clipEnvelope.jumpTo(0);
                    animation.impl_stop();
                } else if(fromDuration(animation.getCurrentTime()) != 0) {
                ...
                if (status != Status.STOPPED) {
                    animation.clipEnvelope.jumpTo(Math.round(durations[i] * rates[i]));
                    animation.impl_stop();
                } else if (fromDuration(animation.getCurrentTime()) != durations[i]) {
