...
public class Tool {
    ...
    private static String readTextFile(String file) {
        InputStream is = Tool.class.getResourceAsStream(file);
    ...
    private static void processModule(JsonObject module, JsonObject variables) {
        ...
        String nagaSrc = resolveVariables(module, "naga-src", variables, module);
        ...
        try (FileSystem zipfs = FileSystems.newFileSystem(zip_disk, zip_properties)) {
            for (int i = 0; i < includes.size(); i++)
                processInclude(includes.getString(i), zipfs, jdkModule, nagaSrc, relocations, relocationKeys, excludes, replacements);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }

    private static void processInclude(String includePattern, FileSystem zipfs, String jdkModule, String nagaSrc, JsonObject relocations, JsonArray relocationKeys, JsonArray excludes, JsonObject replacements) throws IOException {
        /* Path inside ZIP File */
        int p = includePattern.indexOf('*');
        if (p == -1) {
            Path javaFilePath = zipfs.getPath("/" + jdkModule + "/" + includePattern);
            processJavaFilePath(javaFilePath, includePattern, nagaSrc, relocations, relocationKeys, excludes, replacements);
        } else {
            ...
            Files.walkFileTree(searchRootPath, new FileVisitor<Path>() {
                ...
                public FileVisitResult visitFile(Path file, BasicFileAttributes attrs) throws IOException {
                    processJavaFilePath(file, file.toString().substring(relativePathStartIndex), nagaSrc, relocations, relocationKeys, excludes, replacements);
    ...
    private static void processJavaFilePath(Path javaFilePath, String relativePath, String nagaSrc, JsonObject relocations, JsonArray relocationKeys, JsonArray excludes, JsonObject replacements) throws IOException {
        ...
        Path nagaFilePath = Paths.get(nagaSrc, relocatedPath);
        Files.createDirectories(nagaFilePath.getParent());
        ...
        System.out.println();
        Files.write(nagaFilePath, content.getBytes());
    }
