 ...
 * @author Bruno Salmon
 */
final class Elemental2WebEnginePeer extends WebEnginePeerBase {

    // Note - WebFX convention: if webEngine.getWebView() is null, it's a webEngine that applies to the global window
    private final WebEngine webEngine;

    public Elemental2WebEnginePeer(WebEngine webEngine) {
    ...
    public Object executeScript(String script) {
        ...
        if (scriptWindow != null) {
            if ("window".equals(script))
                return Elemental2JSObject.wrapJSObject(scriptWindow);
