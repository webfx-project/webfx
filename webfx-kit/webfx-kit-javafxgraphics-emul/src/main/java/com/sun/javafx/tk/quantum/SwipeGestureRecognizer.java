...
public class SwipeGestureRecognizer implements GestureRecognizer {
    ...
    public static EventTarget CURRENT_TARGET;
    ...
    public void notifyBeginTouchEvent(long time, int modifiers, boolean isDirect,
                                      int touchEventCount) {
        //webfx.platform.shared.services.log.Logger.log("notifyBeginTouchEvent()");
        tracker.params(modifiers, isDirect);
    }

    @Override
    public void notifyNextTouchEvent(long time, /*int*/ String type, long touchId,
                                     int x, int y, int xAbs, int yAbs) {
        //webfx.platform.shared.services.log.Logger.log("notifyNextTouchEvent(" + type + ", " + touchId + ")");
        switch(type) {
            case "PRESSED": //TouchEvent.TOUCH_PRESSED:
                tracker.pressed(touchId, time, x, y, xAbs, yAbs);
                break;
            case "STATIONARY": //TouchEvent.TOUCH_STILL:
                /* NOBREAK */
            case "MOVED": //TouchEvent.TOUCH_MOVED:
                tracker.progress(touchId, time, xAbs, yAbs);
                break;
            case "RELEASED": //TouchEvent.TOUCH_RELEASED:
    ...
    public void notifyEndTouchEvent(long time) {
        //webfx.platform.shared.services.log.Logger.log("notifyEndTouchEvent()");
        // nothing to do
    }
    ...
    private EventType<SwipeEvent> calcSwipeType(TouchPointTracker tracker) {
        ...
        int swipeMaxDuration = 300; //Integer.getInteger("com.sun.javafx.gestures.swipe.maxduration", 300);
...
*/
        EventUtil.fireEvent(CURRENT_TARGET,
        new SwipeEvent(null, CURRENT_TARGET, swipeType,
                cc.getX(), cc.getY(),
                cc.getAbsX(), cc.getAbsY(),
                false, //(modifiers & KeyEvent.MODIFIER_SHIFT) != 0,
                false, //(modifiers & KeyEvent.MODIFIER_CONTROL) != 0,
                false, //(modifiers & KeyEvent.MODIFIER_ALT) != 0,
                false, //(modifiers & KeyEvent.MODIFIER_WINDOWS) != 0,
                isDirect,
                touchCount,
                null)
        );
