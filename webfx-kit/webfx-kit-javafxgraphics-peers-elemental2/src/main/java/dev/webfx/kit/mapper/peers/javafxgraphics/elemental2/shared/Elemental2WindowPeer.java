...
public class Elemental2WindowPeer extends WindowPeerBase {
    ...
    public Elemental2WindowPeer(Window window) {
        super(window);
    ...
    public void setBounds(float x, float y, boolean xSet, boolean ySet, float w, float h, float cw, float ch, float xGravity, float yGravity) {
        ...
        if (ySet)
            windowStyle.top = (y - DomGlobal.window.screenY - Elemental2PrimaryStagePeer.windowScreenYCorrection) + "px";
        if (w < 0 && cw > 0)
            w = cw; // + 6;
        if (h < 0 && ch > 0)
            h = ch; // + 6;
