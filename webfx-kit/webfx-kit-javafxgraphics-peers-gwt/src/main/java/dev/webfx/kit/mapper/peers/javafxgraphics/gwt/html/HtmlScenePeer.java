...
public final class HtmlScenePeer extends ScenePeerBase {
    ...
    public void updateParentAndChildrenPeers(Parent parent, ListChangeListener.Change<? extends Node> childrenChange) {
        ...
        // HasNoChildrenPeers have no JavaFX children (mapped to HTML) but they do have HTML children (generated by the peer)
        if (parentPeer instanceof HasNoChildrenPeers) // Ex: HtmlHtmlTextPeer, HtmlHtmlTextEditorPeer, HtmlVisualGridPeer
            return; // => we don't want to clear the HTML children generated by the peer
        Element childrenContainer = parentPeer.getChildrenContainer();
        if (childrenChange == null) { // Indicates it's all children to map and not just a partial change
            List<Element> childElements = toChildElements(parent.getChildren());
            HtmlUtil.setChildren(childrenContainer, childElements);
        } else { // partial change of children
            while (childrenChange.next()) {
                // When children have been removed from the scene graph, we remove their associated elements from the DOM
                ...
                if (childrenChange.wasAdded()) {
                    // Getting the associated elements
                    List<Element> childElementsToAdd = toChildElements(childrenChange.getAddedSubList());
                    // Getting the index we need to insert them from
                    int from = childrenChange.getFrom();
                    NodeList<elemental2.dom.Node> childNodes = childrenContainer.childNodes;
                    // Case when they are appended after all existing children
                    if (from >= childNodes.length) {
                        HtmlUtil.appendChildren(childrenContainer, childElementsToAdd);
                    } else { // Case when they are inserted at a specific index, before some existing children
                        elemental2.dom.Node insertBeforeNode = childNodes.getAt(from);
                        childElementsToAdd.forEach(elementToAdd -> childrenContainer.insertBefore(elementToAdd, insertBeforeNode));
                    }
                }
            }
        }
