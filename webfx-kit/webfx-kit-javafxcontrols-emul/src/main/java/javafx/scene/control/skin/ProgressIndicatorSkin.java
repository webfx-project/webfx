...
public class ProgressIndicatorSkin extends SkinBase<ProgressIndicator> {
    ...
    private final String DONE = "Done"; //ControlResources.getString("ProgressIndicator.doneString");
    ...
    //private TreeShowingExpression treeShowingExpression; // Not supported by WebFX
    ...
    public ProgressIndicatorSkin(ProgressIndicator control) {
        ...
        //registerChangeListener(treeShowingExpression, e -> updateAnimation()); // Not supported by WebFX
        registerChangeListener(control.parentProperty(), e -> updateAnimation()); // Just listening to parent instead, and use Node.impl_treeVisible()
     ...
     */
    private IntegerProperty indeterminateSegmentCount = new StyleableIntegerProperty(/*8*/12) {
        @Override protected void invalidated() {
        ...
        @Override public CssMetaData<ProgressIndicator,Number> getCssMetaData() {
            return null; //INDETERMINATE_SEGMENT_COUNT;
        }
    };
    ...
    void initialize() {
        ...
        if (isIndeterminate) {
            ...
            if (control.impl_isTreeVisible() /*NodeHelper.isTreeShowing(control)*/) {
                if (indeterminateTransition != null) {
    ...
    void updateAnimation() {
        ...
        final boolean isTreeShowing = control.impl_isTreeVisible() /*NodeHelper.isTreeShowing(control)*/ &&
    ...
    private final class IndeterminateSpinner extends Region {
        ...
        private class IndicatorPaths extends Pane {
            @Override protected double computePrefWidth(double height) {
                /*double w = 0; // Commented for WebFX as this code assumes CSS is setting shapes
                ...
                return w;*/ return 2000; // arbitrary big size (will be scaled down)
            }

            @Override protected double computePrefHeight(double width) {
                /*double h = 0; // Commented for WebFX as this code assumes CSS is setting shapes
                for(Node child: getChildren()) {
                    if (child instanceof Region) {
                        Region region = (Region)child;
                        *//*if (region.getShape() != null) {
                ...
                return h;*/ return 2000; // arbitrary big size (will be scaled down)
            }
